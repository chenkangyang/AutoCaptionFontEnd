...
class AudioSection():
    ...
    def end_point_div(self, min_voice, space):
        ...
        # 计算对应的音频段落
        index = findIndex(vol, threshold, min_voice, space) * \
            (self.nframes*1.0/len(vol)/self.framerate)
    ...
    def get_text(self, index):
        ...
        for i in range(l):
            ...
            result = aipSpeech.asr(get_file_content(
                par_path), 'wav', 16000, {'lan': 'zh'})
    ...
    def draw_picture(self, index, threshold):
        ...
        # 讲音量对应的帧数转化成时间轴上的坐标
        frame = np.arange(0, len(vol)) * (self.nframes *
                                          1.0/len(vol)/self.framerate)
